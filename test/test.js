...
contract("dBank", ([deployer, user]) => {
    ...
    describe("testing token contract...", () => {
        describe("success", () => {
            it("checking token name", async () => {
                expect(await token.name()).to.be.eq("Star Coin");
            });

            it("checking token symbol", async () => {
                expect(await token.symbol()).to.be.eq("STAR");
            });
    ...
    describe("testing deposit...", () => {
        ...
        describe("failure", () => {
            it("depositing should be rejected when less than 0.01 ETH", async () => {
                await dbank
                    .deposit({ value: 10 ** 15, from: user })
                    .should.be.rejectedWith(EVM_REVERT); //too small amount
            });

            it("depositing should be rejected when user has already deposited", async () => {
                await dbank.deposit({ value: 10 ** 16, from: user }); //0.01 ETH
                await dbank
                    .deposit({ value: 10 ** 16, from: user })
                    .should.be.rejectedWith(EVM_REVERT);
            });
        });
    });
